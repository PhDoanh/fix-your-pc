...
void Interface::loadBackground(const std::string &path)
{
	...
	if (bg_surface == nullptr)
		console.error();
	bg_texture = SDL_CreateTextureFromSurface(Game::renderer, bg_surface);
	SDL_FreeSurface(bg_surface);
	if (bg_texture == nullptr)
		console.error();

	if (SDL_RenderCopy(Game::renderer, bg_texture, nullptr, nullptr) != 0)
		console.error();
...
void Interface::loadAvatar(const std::string &path)
{
	...
	if (ava_surface == nullptr)
		console.error();
	ava_texture = SDL_CreateTextureFromSurface(Game::renderer, ava_surface);
	...
	if (SDL_QueryTexture(ava_texture, NULL, NULL, &ava_width, &ava_height) != 0)
		console.error();
	int x = (Game::win_width - ava_width) / 2;
	int y = (Game::win_height - ava_height) / 3;

	SDL_Rect dstRect = {x, y, ava_width, ava_height};

	if (SDL_RenderCopy(Game::renderer, ava_texture, nullptr, &dstRect) != 0)
		console.error();
	SDL_RenderPresent(Game::renderer);
}
...
void Interface::showInputBar()
{
	...
	SDL_Texture *txt_texture = SDL_CreateTextureFromSurface(Game::renderer, txt_surface);
	SDL_FreeSurface(txt_surface);

	int txt_width, txt_height;
	SDL_QueryTexture(txt_texture, nullptr, nullptr, &txt_width, &txt_height);
	int x = (Game::win_width - txt_width) / 2;
	int y = (Game::win_height - txt_height) / 2;
	SDL_Rect txt_rect = {x, y, txt_width, txt_height};

	SDL_RenderCopy(Game::renderer, txt_texture, nullptr, &txt_rect);
